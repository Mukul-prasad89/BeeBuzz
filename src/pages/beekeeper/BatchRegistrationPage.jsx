...
export default function BatchRegistrationPage() {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      setResult(res)
      setStep(2)
      addToast(t('batchRegistration.toastSuccess'))
    } catch {
      addToast(t('batchRegistration.toastError'), 'error')
    }
    setSubmitting(false)
  }
  ...
  return (
    <div className="max-w-2xl mx-auto space-y-6">
      <h1 className="text-2xl font-bold font-heading text-charcoal-800">{t('batchRegistration.title')}</h1>

      <StepWizard steps={[t('batchRegistration.steps.details'), t('batchRegistration.steps.review'), t('batchRegistration.steps.success')]} currentStep={step} />

      {/* Step 0: Harvest Details */}
      {step === 0 && (
        <Card>
          <h3 className="font-bold text-charcoal-800 font-heading mb-4">{t('batchRegistration.step1')}</h3>

          <div className="mb-4">
            <label className="section-label mb-2 block">{t('batchRegistration.form.selectHives')}</label>
          ...
          <div className="grid sm:grid-cols-2 gap-4 mb-4">
            <div>
              <label className="section-label mb-1 block">{t('batchRegistration.form.harvestDate')}</label>
              <input type="date" value={form.harvestDate} onChange={(e) => setForm({ ...form, harvestDate: e.target.value })} className="input" />
            </div>
            <div>
              <label className="section-label mb-1 block">{t('batchRegistration.form.honeyType')}</label>
            ...
            <div>
              <label className="section-label mb-1 block">{t('batchRegistration.form.quantity')}</label>
            ...
            <div>
              <label className="section-label mb-1 block">{t('batchRegistration.form.moisture')}</label>
          ...
          <div className="mb-4">
            <label className="section-label mb-1 block">{t('batchRegistration.form.notes')}</label>
            <textarea value={form.notes} onChange={(e) => setForm({ ...form, notes: e.target.value })} className="input" rows={2} placeholder={t('batchRegistration.form.notesPlaceholder')} />
          </div>

          <Button onClick={() => setStep(1)} disabled={form.hiveIds.length === 0 || !form.quantityKg} className="w-full">
            {t('batchRegistration.form.continue')}
          </Button>
        </Card>
      )}
      ...
      {step === 1 && (
        <Card>
          <h3 className="font-bold text-charcoal-800 font-heading mb-4">{t('batchRegistration.step2')}</h3>
          <div className="space-y-3 mb-6">
            <div className="flex justify-between py-2 border-b border-charcoal-100">
              <span className="text-sm text-charcoal-500">{t('batchRegistration.review.hives')}</span>
              <span className="text-sm font-medium text-charcoal-800">{form.hiveIds.join(', ')}</span>
            </div>
            <div className="flex justify-between py-2 border-b border-charcoal-100">
              <span className="text-sm text-charcoal-500">{t('batchRegistration.review.date')}</span>
              <span className="text-sm font-medium text-charcoal-800">{form.harvestDate}</span>
            </div>
            <div className="flex justify-between py-2 border-b border-charcoal-100">
              <span className="text-sm text-charcoal-500">{t('batchRegistration.review.honeyType')}</span>
              <span className="text-sm font-medium text-charcoal-800">{form.honeyType}</span>
            </div>
            <div className="flex justify-between py-2 border-b border-charcoal-100">
              <span className="text-sm text-charcoal-500">{t('batchRegistration.review.quantity')}</span>
              <span className="text-sm font-medium text-charcoal-800">{form.quantityKg} kg</span>
            </div>
            <div className="flex justify-between py-2">
              <span className="text-sm text-charcoal-500">{t('batchRegistration.review.moisture')}</span>
              <span className="text-sm font-medium text-charcoal-800">{form.moisturePct || t('batchRegistration.review.na')}%</span>
            </div>
          </div>
          ...
          {submitting ? (
            <div className="text-center py-8">
              ...
              <p className="text-sm text-charcoal-500">{t('batchRegistration.loading')}</p>
            </div>
          ) : (
            <div className="flex gap-3">
              <Button variant="secondary" onClick={() => setStep(0)} className="flex-1">{t('batchRegistration.back')}</Button>
              <Button onClick={handleSubmit} className="flex-1">{t('batchRegistration.registerMint')}</Button>
            </div>
          )}
        </Card>
      )}
      ...
      {step === 2 && result && (
        <Card>
          <div className="text-center mb-6">
            ...
            <h3 className="text-xl font-bold text-charcoal-800 font-heading">{t('batchRegistration.successTitle')}</h3>
            <p className="text-sm text-charcoal-400 mt-1">{t('batchRegistration.successDesc')}</p>
          </div>

          <div className="text-center mb-6">
            <p className="section-label mb-1">{t('batchRegistration.batchId')}</p>
          ...
          <div className="flex gap-3 mb-6">
            <a href={result.qrCodeUrl} download className="btn-secondary flex-1 flex items-center justify-center gap-2">
              <Download className="h-4 w-4" /> {t('batchRegistration.downloadQr')}
            </a>
            <button onClick={() => window.print()} className="btn-secondary flex-1 flex items-center justify-center gap-2">
              <Printer className="h-4 w-4" /> {t('batchRegistration.printLabel')}
            </button>
          </div>
          ...
          <div className="flex gap-3 mt-6">
            <Button variant="secondary" onClick={() => { setStep(0); setResult(null); setForm({ hiveIds: [], harvestDate: new Date().toISOString().split('T')[0], honeyType: 'Wildflower Honey', quantityKg: '', moisturePct: '', notes: '' }) }} className="flex-1">
              {t('batchRegistration.registerAnother')}
            </Button>
            <Button onClick={() => navigate('/beekeeper')} className="flex-1">
              {t('batchRegistration.goToDashboard')}
            </Button>
          </div>
        </Card>
      )}
